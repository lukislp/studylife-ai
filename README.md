...
| `INTERNAL_API_PORT`            | `8001`                    | Audit O6-ai: second port `/internal/*` is served on (in addition to the main port, for one transition release) — a second `uvicorn.Server` in the same process. Only meaningful for the k3s deployment's `NetworkPolicy`; harmless to leave at the default everywhere else. See [Deployment](#deployment). |
