...
- **Incremental sync strategy**: `GET /api/notes` has no `updatedSince`/pagination filter, and `NoteDto` has no soft-delete/tombstone field — hard deletes leave no sync trace. Two options, not yet decided: (a) `studylife-ai` polls the full note list and diffs client-side against the last known state (Qdrant point IDs/hashes), or (b) StudyLife itself gets an `updatedSince` query param added to `NotesController.GetAll`. (a) needs zero changes to the other repo but re-transfers the full note list every sync; (b) is more efficient but is a change to StudyLife, not this repo — the user's call whether to make it. Needs deciding when M2 gets to the sync-loop design.
- Same open question applies to `api/sessions` (no `UpdatedAt` field on `StudySessionDto` at all) if session data ends up in scope for ingestion too.
